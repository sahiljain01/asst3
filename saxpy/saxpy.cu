...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    double startTimeOnlyComputation = CycleTimer::currentSeconds();
    saxpy_kernel<<<blocks, threadsPerBlock>>>(N, alpha, device_x, device_y, device_result);
    cudaDeviceSynchronize();
    double endTimeOnlyComputation = CycleTimer::currentSeconds();
    ...
    double computeDuration = endTimeOnlyComputation - startTimeOnlyComputation;

    printf("Effective BW by CUDA saxpy: %.3f ms\t\t[%.3f GB/s]\n", 1000.f * overallDuration, GBPerSec(totalBytes, overallDuration));
    printf("Compute Duration: %.3f ms\t\t[%.3f GB/s]\n", 1000.f * computeDuration, GBPerSec(totalBytes, computeDuration));
